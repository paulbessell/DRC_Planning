...
library(DT); library(leaflet); library(flextable); library(leaflet.extras); library(plotly)
...
riversMapping$lab <- paste0("<b>", "ZS = ", riversMapping$plZSAdj,
                            "<br/>", "Province = ", riversMapping$fProvince, 
                              "<br/>", "Coordination = ", riversMapping$Coordtion, 
...
```{r Carte des interventions, echo = F, out.width='100%'}
  ...
  leaflet(riversMappingNew) %>%
    addTiles(group = "Streets") %>%
        ...
        addPolygons(data = coordinations,
                fill = NA,
                color = "black",
                opacity = 1.,
                weight = 2,
  ...
  fitBounds(bounds[1], bounds[2], bounds[3], bounds[4]) %>%
    addLayersControl(
      overlayGroups = c("New Interventions", "Current Interventions", "Non-Candidate", "Community sites", "Case heat map", "Province", "Coordination"),
      options = layersControlOptions(collapsed = FALSE)
    ) %>%
    hideGroup(c("Non-Candidate", "Province"))
...
DT::datatable(ZSRiverDT, filter = 'top', extensions = c('Scroller', 'Buttons', 'ColReorder'),
                            ...
                            selection = "none",
              colnames = c("RiverID", "ZS", "Province", "Coordination", "Deploiment", "Debit", "Cas total", "Cas pondérés", "Long. rivière", "Cas / km", "Pondérés / km", "Status", "Rank", "Total river (km)"))
```

```{r priority chart, echo = F, eval = params$`Inclure des sites fluviaux`, message = FALSE, warning = FALSE}

zsRiverPlot <- ZSRiverOutput %>%
  st_drop_geometry() %>%
  ungroup() %>%
  filter(Status != "Non-candidate") %>%
  arrange(desc(casesKm)) %>%
  mutate(cumLength = cumsum(RiverLength),
         cumCases = cumsum(CasesTotal),
         rowID = row_number(),
         label = paste0(Status, "\n", plZSAdj, "\n", fProvince, "(", Coordtion, ")", "\n", "Discharge = ",  round(Discharge,1), "\n", "Cases = ", round(CasesTotal, 1), "\n", "Length = ", round(RiverLength, 1))) 


plotCases <- ggplot(zsRiverPlot, aes(x = rowID, y = cumLength, width = (sqrt(CasesTotal / max(CasesTotal)) * 1.5) + 0.2, fill = Coordtion, text = label)) + 
  geom_col() + 
  scale_fill_manual(values = c("BDDS" = "darkblue", "BDDN" = "blue2", "KOR" = "green4", "KOCC" = "green3")) +
  theme(legend.position = "top") +
  geom_point(data = zsRiverPlot %>% filter(Status == "New site"), aes(x = rowID, y = cumLength + 100), shape = 12) +
  theme(axis.title.x = element_blank()) +
  ylab("Cumulative river length (km)")

ggplotly(plotCases, tooltip = "text") %>%
  layout(legend = list(
    orientation = "h",
    y = -0.1,
    x = 0))
