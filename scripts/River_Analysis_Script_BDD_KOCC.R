...
aggregatedData <- ifelse(exists("params"), params$`Analyse les donnes aggrege 2019-2023`, TRUE)
...
if(aggregatedData){
  ...
  cases <- read_excel("data/CaseData/complet/Extraction_PNLTHA_DRC_2019_2023_cas_cumules.xlsx") %>%
    mutate(cCaseID = row_number()) %>%
    filter(!is.na(Longitude),
           Province %in% c("BDD NORD", "BDD SUD", "Kasaï Occidental", "Kasaï Oriental")) %>%
  ...
  load("data/RImages/distdfsort2_5k_AGG.RData")
  ...
  cases_sf_cluster <- cases_sf %>%
    ...
    mutate(ClusterID = ifelse(is.na(ClusterID), paste0("s", CaseID), ClusterID)) %>%
    group_by(ClusterID) %>%
    ...
    group_by(ClusterID) %>%
    mutate("nVillages" = n()) %>%
    dplyr::select(-c("Province", "PROVINCE.x")) %>%
